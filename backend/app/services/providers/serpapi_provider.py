"""SerpAPI image search provider (Google Images). Fallback and film-stills queries."""
import logging
import os
...
class SerpApiProvider(BaseImageProvider):
    """Image search via SerpAPI Google Images. 100 free searches/month, then paid."""
    ...
    async def search(
        ...
        if not self.api_key:
            logger.warning("SERPAPI_KEY/SEARCH_API_KEY not set; returning empty results")
            return []
